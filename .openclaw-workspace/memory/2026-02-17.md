...
## v2 Plan Review — Bugs Found

- Identified 10 issues in mac-mini-full-plan.md v2 after Kamil asked for review
- Critical: launchd plist $HOME doesn't expand in XML (need absolute paths)
- Critical: PATH not set in launchd env (openclaw/claude commands won't be found)
- Bug: `git add -A` in backup section violates AGENTS.md safety rules
- Bug: bot token grep fragile with pretty-printed JSON
- Missing: UPS recommendation, log rotation, network-down alerting, timezone config
- Missing: Phase 3→4 ordering conflict (onboard creates defaults, then overwritten)
- Missing: full ~/.openclaw backup (not just workspace)
- Kamil asked if should fix to v3 or roll with v2 — awaiting answer
- All 3 guide files sent to Kamil via Telegram as documents
